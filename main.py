...
# Optional imports for content extraction & ranking.
...
def style_entry(e, pal):
    e.configure(
        ...
        insertbackground=pal["text"],  # caret color
...
def prevent_blank_select(listbox: tk.Listbox):
    def _on_click(e):
        lb = e.widget
        # nearest() gives last index if you click below; we must verify y is inside that row's bbox
        idx = lb.nearest(e.y)
        if idx < 0:
            return "break"
        bbox = lb.bbox(idx)  # (x, y, w, h) for that row
        if not bbox:
            return "break"
        x, y, w, h = bbox
        if e.y < y or e.y > y + h:
            # Click is outside all row rectangles -> block default selection and clear any highlight
            lb.selection_clear(0, tk.END)
            return "break"
        # else: inside a real row -> allow normal behavior
...
class ContentSearchEngine:
    """
    Builds a light-weight content index of files in the currently opened folder
    and ranks results using TF–IDF + cosine similarity. Falls back to substring
    matching when sklearn is unavailable.
    """
    SUPPORTED_EXTS = {
    ...
    def __init__(self, cache_path=CONTENT_CACHE_PATH):
        ...
        self.folder = None
        self.files_meta = []  # [{'path':..., 'ext':..., 'mtime':..., 'size':...}, ...]
        self.texts = []       # extracted text aligned with files_meta
    ...
    def _extract_text(self, meta):
        ...
        try:
            ...
            # ----------------------------------------------------
            # 🔥 NEW: IMAGE OCR (jpg/png/jpeg/webp/bmp/gif)
            # ----------------------------------------------------
            if ext in ('.png', '.jpg', '.jpeg', '.bmp', '.gif', '.webp'):
                if Image is not None and pytesseract is not None:
                    ...
                    return ""  # OCR unavailable → skip image text
            # ----------------------------------------------------
    ...
    def query_scores(self, query):
        """
        Returns a dict: path -> score for content-based relevance.
        When sklearn is not available, does a simple case-insensitive substring
        frequency score across extracted text.
        """
...
class RulesPanel(tk.Frame):
    def __init__(self, master, store):
        ...
        # bug fix: clicking blank space selects last item
    ...
    def _style_buttons_recursive(self, widget, pal):
        for c in widget.winfo_children():
            if isinstance(c, tk.Button):
                # highlight "danger" ones like Delete / Remove
                txt = c.cget("text").lower()
...
class RuleEditor(tk.Toplevel):
    def __init__(self, parent, rule_index, store, on_save):
        ...
        # register so MainApp can re-theme us on toggle
...
class FileExplorerPanel(tk.Frame):
    def __init__(self, master, store):
        ...
        self._index_status_var = tk.StringVar(value='')  # small status hint
        # --- Content Search Added ---
        ...
        # --- Content Search Added ---
        # small line for index status; it won't change layout
    ...
    def refresh_list(self):
        ...
        if q:
            ...
            def base_name_score(r):
                name = r['name'].lower()
                if q in name:
                    # crude filename relevance: favor prefix, then substring
                    if name.startswith(q):
                        return 1.0
                    return 0.6
                return 0.0

            # merge: filename score + content score (weighted)
            scored = []
            for r in rows:
                s_name = base_name_score(r)
                s_content = content_scores.get(r['path'], 0.0)
                # weight content slightly higher to honor semantic matches
                score = s_name + (1.2 * s_content)
                if s_name > 0 or s_content > 0:
                    scored.append((score, r))

            # If none matched (e.g., index not ready), fall back to filename filter
            if scored:
        ...
        # Sort: if there's a query, we've already sorted by relevance above.
    ...
    def apply_palette(self, pal: dict):
        ...
        # explicit theming so dark mode stays readable
        try:
...
class PreviewPanel(tk.Frame):
    def __init__(self, master):
        ...
        self._photo = None          # tk image
        self._photos = []           # for multi-page (PDF)
        self._orig_image = None     # original PIL image for normal images
    ...
    def _render_single_image(self, path: str):
        ...
        if base:
            ...
            # keep existing title if no path passed (resize only)
            pass
    ...
    def _on_content_resize(self, event=None):
        # When the panel is resized and the current preview is a normal image,
        # re-render it to fit the new size.
        if self._orig_image is not None:
            # pass empty path so we don't change the title text
            self._render_single_image("")

    # ---------- Text preview ----------
    def show_text_preview(self, path: str, content: str):
        self._orig_image = None  # no image active
        self._clear_content()
...
class FolderPanel(tk.Frame):
    ...
    def apply_palette(self, pal: dict):
        ...
        # details card with crisp outline
        try:
...
class MainApp(tk.Tk):
    def __init__(self):
        ...
        def _toggle_theme():
            ...
            # re-style the theme toggle button too
